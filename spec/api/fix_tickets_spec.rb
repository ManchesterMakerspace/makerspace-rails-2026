...
RSpec.describe 'Fix tickets', type: :request do
  ...
  path '/fix_tickets' do
    get 'List authorized repair tickets' do
      ...
      response '200', 'Scoped, filtered, sorted page; public-only readers require current membership' do
        schema type: :object, required: %w[tickets total page pageSize], properties: {
          tickets: { type: :array, items: { type: :object } }, total: { type: :integer }, page: { type: :integer }, pageSize: { type: :integer }
        }
        run_test!
      end
    end
    post 'Submit a report (activeMember and future expiration; cap enforced)' do
      ...
      response('200', 'Created, or previously accepted idempotent submission') { run_test! }
    end
  end
  path '/fix_tickets/catalog' do
    get 'Report catalog, creation capability, cap and current open count' do
      tags 'Fix tickets'
      produces 'application/json'
      response('200', 'Catalog and capabilities') { run_test! }
    end
  end
  path '/fix_tickets/{id}' do
    ...
    get 'Read a ticket and redacted history' do
      tags 'Fix tickets'
      produces 'application/json'
      response('200', 'No reporter identity is included') { run_test! }
    end
    patch 'Update authorized ticket fields; priority is immutable' do
      ...
      response('200', 'Updated; operation-specific staff/assignee/reporter authorization applies') { run_test! }
    end
  end
  { notes: ['Append a note', { note: { type: :string } }], withdraw: ['Withdraw own nonterminal report', {}],
    ...
    path "/fix_tickets/{id}/#{action}" do
      ...
      post title do
        ...
        response '403', 'Caller lacks this action capability' do; end
        response '422', 'Validation, cap, revision, or lifecycle conflict' do; end
      end
    end
  end
  path '/fix_tickets/{id}/assignee_options' do
    ...
    get 'Repair staff: search eligible assignees' do
      ...
      response('200', 'Up to fifty active unexpired members') { let(:member) { create(:member, :admin, :current) }; run_test! }
    end
  end
