...
RSpec.describe 'Admin checkins', type: :request do
  ...
  context 'as a regular member' do
    ...
    it 'ignores requested cards not held by the member and masks returned UIDs' do
      request_checkins([other_card.uid])

      expect(response).to have_http_status(:ok)
      uids = JSON.parse(response.body).fetch('checkins').map { |checkin| checkin.fetch('uid') }
      expect(uids).to contain_exactly(member_card.id.to_s)
      expect(uids).not_to include(member_card.uid, other_card.uid, other_card.id.to_s)
    end
  end

  shared_examples 'privileged checkin access' do |role|
    let(:actor) { create(:member, role => true) }

    before { sign_in actor }

    it "allows #{role} members to query the requested UID range" do
      request_checkins([member_card.uid, other_card.uid])

      expect(response).to have_http_status(:ok)
      uids = JSON.parse(response.body).fetch('checkins').map { |checkin| checkin.fetch('uid') }
      expected_uids = role == :resource_manager ? [member_card.id.to_s, other_card.id.to_s] : [member_card.uid, other_card.uid]
